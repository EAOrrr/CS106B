...
int factorial(int n) {
    if(n < 0){
        error("invalid calls on negative value ");
    }
    else if (n == 0) {
        return 1;
    } else {
...
STUDENT_TEST("factorial on negative numbers"){
    EXPECT_ERROR(factorial(-3));
}

STUDENT_TEST("myPower() on negative and positive inputs"){
    for(int base = -3; base < 3; base++){
        for(int exp = -3; exp < 3; exp++){
            cout << "base: " << base << " exp: "<< exp <<endl;
            cout << myPower(base, exp) << ' ' <<  pow(base, exp) << endl;
            EXPECT_EQUAL(myPower(base, exp), pow(base, exp));
        }
    }
}
