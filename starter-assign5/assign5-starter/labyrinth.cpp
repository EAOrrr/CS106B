 ...
 */
const string kYourName = "Zhixin Cao";

/* Change this constant to the path out of your labyrinth. */
const string kPathOutOfNormalMaze = "SSENSEESWWEENNNWW";
...
bool isPathToFreedom(MazeCell* start, string moves, Set<string> needs) {
    ...
    for(int i = 0, len  = moves.length(); i <= len; ++i){
        // cur = nullptr: invalid step
        if(cur == nullptr) return false;
        // get the current item->do not need that item
        if(needs.contains(cur->contents)){
            needs.remove(cur->contents);
        }
        // get all the needed item
        ...
        if(i < len){
            ...
            if(ch == 'N'){
                ...
                error("invalid Character" + moves[i]);
            }
        }
    }
    return false;
