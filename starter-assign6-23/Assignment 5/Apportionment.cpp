...
Map<string, int> apportion(const Map<string, int>& populations, int numSeats) {
    ...
    if(populations.isEmpty()) error("No State!");
    if(numSeats < populations.size()) error("seats less than states");
    HeapPQueue priority;    Map<string, int> result;
    for(string state: populations){
        if(populations[state] <= 0) error("states with population less than 0");
        result.put(state, 1);
        priority.enqueue({state, -populations[state]/sqrt(2)});
        DataPoint hey = priority.peek();
//        std::cout << state << hey.name << " " << hey.weight << std::endl;
    }
    numSeats -= populations.size();
    while(numSeats > 0){
        string winState = priority.dequeue().name;
        result[winState]++;
        int factor = result[winState];
        priority.enqueue({winState, -populations[winState]/sqrt(factor * (factor+1))});
        numSeats--;
    }
    return result;
}
...
STUDENT_TEST("fOR MORE TEST"){

    Map<string, int> populations1 = {
                                    { "House",         435 },
                                    { "Senate",        100 },
                                    { "Supreme Court",   0 },
                                    { "President",       1 },
    ...
    Map<string, int> populations2 = {
                                    { "House",         435 },
                                    { "Senate",        -100 },
                                    { "Supreme Court",   9 },
                                    { "President",       1 },
                                    };
    EXPECT_ERROR(apportion(populations1, 9));
    EXPECT_ERROR(apportion(populations2, 9));


}

STUDENT_TEST("For questions"){
    auto populations2020 = populationsFrom("res/apportionment/2020.csv");
    auto populations26 = populationsFrom("res/apportionment/2020-MNDown26.csv");
    auto populations89 = populationsFrom("res/apportionment/2020-NYUp89.csv");
    auto result2020 = apportion(populations2020, 435);
    auto result26 = apportion(populations26, 435);
    auto result89 = apportion(populations89, 435);
    cout << "2020 New York: " << result2020["New York"] << "Minnesota: " << result2020["Minnesota"] << endl;
    cout << "2020-26 New York: " << result26["New York"] << "Minnesota: " << result26["Minnesota"] << endl;
    cout << "2020+89 New York: " << result89["New York"] << "Minnesota: " << result89["Minnesota"] << endl;
}
