...
void HeapPQueue::enqueue(const DataPoint& data) {
    /* TODO: Delete the next line and implement this. */
    // check the capacity of elems
    if(logicalSize+1 >= allocatedSize){
        // need to be expanded
        DataPoint* temp = new DataPoint[allocatedSize * 2];
    ...
    int curIndex = logicalSize+1;
    elems[curIndex]= data;
    while(curIndex > 1){
        // data has parent
        // compare the weight of data and its parent
        if(data.weight >= elems[curIndex/2].weight) break;
        // swap the index of parent and data
        elems[curIndex] = elems[curIndex/2];
        elems[curIndex/2] = data;
        curIndex = curIndex/2;
    }
    logicalSize++;
}
...
DataPoint HeapPQueue::peek() const {
    /* TODO: Delete the next line and implement this. */
    if(isEmpty()) error("HeapQueue empty!");
    return elems[1];
}
...
DataPoint HeapPQueue::dequeue() {
    ...
    DataPoint min = peek();
    int currIndex = 1;
    elems[currIndex] = elems[logicalSize];
    while(currIndex * 2 <= logicalSize){
        // elems[currIndex] still has child
        int minChild;
        if(currIndex * 2 + 1 > logicalSize){
            // has only left child
            minChild = currIndex * 2;
        }
        else{
            DataPoint left = elems[currIndex*2], right = elems[currIndex*2+1];
            if(elems[currIndex*2].weight <= elems[currIndex*2+1].weight) minChild = currIndex*2; // left child
            else minChild = currIndex*2+1;
        }
        if(elems[currIndex].weight <= elems[minChild].weight) break;
        DataPoint temp = elems[currIndex];
        elems[currIndex] = elems[minChild];
        elems[minChild] = temp;
        currIndex = minChild;
    }
    logicalSize--;
...
STUDENT_TEST("Handles many data points with negative weights reversed order.") {
    HeapPQueue pq;
